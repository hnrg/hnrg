...
describe('Appointment APIs', () => {
  ...
  test('Should load available appointments for a date', async () => {
    const response = await request(server).get(`/api/turnos/${moment().add(1, 'days').format('YYYY-MM-DD')}`);
    expect(response.statusCode).toBe(200);
  }, 5000);

  test('Should load all available appointments', async () => {
    const response = await request(server).get('/api/turnos');
    expect(response.statusCode).toBe(200);
  }, 5000);

  test('Should load available appointments for a previous date', async () => {
    const response = await request(server).get(`/api/turnos/${moment().add(-1, 'days').format('YYYY-MM-DD')}`);
